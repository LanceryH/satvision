...
class Mission_class:
    def __init__(self,
                 ...
                 ALTITUDE="undefined",
                 ROCKET="undefined"):
        self.MISSION_CHAR={"SaxaVord": {"perigee_alt":340.0,
                                    "apogee_alt":340.0,
                                    "inclination":90.0,
                                    "latitude":60.8},
                           "Baikonour":{"perigee_alt":410.0,
                                        "apogee_alt":410.0,
                                        "inclination":51.6,
                                        "latitude":46.0},
                           "Kourou":{"perigee_alt":300.0,
                                       "apogee_alt":35786.0,
                                       "inclination":5.2,
                                       "latitude":5.2},
                           "Cap Canaveral":{"perigee_alt":1681.0,
                                   "apogee_alt":1681.0,
                                   "inclination":103.0,
                                   "latitude":28.5}}
        self.ROCKET = ROCKET
        ...
        self.CLIENT = CLIENT
        self.PERIGEE = self.MISSION_CHAR[CLIENT]["perigee_alt"]+p.R_e
        self.APOGEE = self.MISSION_CHAR[CLIENT]["apogee_alt"]+p.R_e
    ...
    def build(self, error_min=1e-6, b_last=3, pas=1):
        ...
        self.DV1 = self.ROCKET.STAGES[0].ISP*p.g0*np.log(self.MASS_INITIAL_1/self.MASS_FINAL_1)
        ...
        if len(self.ROCKET.STAGES)>1 and len(self.ROCKET.STAGES)<3:
            ...
            self.DV2 = self.ROCKET.STAGES[1].ISP*p.g0*np.log(self.MASS_INITIAL_2/self.MASS_FINAL_2)
        ...
        if len(self.ROCKET.STAGES)>2:
            ...
            self.DV3 = self.ROCKET.STAGES[2].ISP*p.g0*np.log(self.MASS_INITIAL_3/self.MASS_FINAL_3)

        self.DV = self.DV1 + self.DV2 + self.DV3
        self.LOSSES = 2.452e-3*self.ALTITUDE**2 + 1.051*self.ALTITUDE+1387.5
        self.V_INITIAL = p.Omega_e*p.R_e*np.cos(np.deg2rad(self.LATITUDE))*np.sin(np.deg2rad(self.AZIMUTH))/86.4
        self.V_FINAL = self.V_INITIAL + self.DV - self.LOSSES
        self.VF = np.sqrt(p.GM_e/self.PERIGEE)*1000
        ...
        if len(self.ROCKET.STAGES)==2:
            ...
            DeltaV_r=p.g0*ISP1*np.log(b1)+p.g0*ISP2*np.log(b2)
            h_list=[]
            h=pas
            sign_0=1
        ...
        if len(self.ROCKET.STAGES)==3:
            ...
            DeltaV_r = p.g0*ISP1*np.log(b1)+p.g0*ISP2*np.log(b2)+p.g0*ISP3*np.log(b3)
            h_list=[]
            h=pas
            sign_0=1
            while np.abs(self.DV_REQUIRED-DeltaV_r)>error_min:
        ...
        self.SEMI_MAJOR_AXIS = 1/((2/self.PERIGEE) - ((self.V_FINAL/1000)**2/p.GM_e))
